...
export function CollaboratorsCard({
  ...
  const field = 'border border-slate-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500';
    ...
    <div className="bg-white rounded-2xl border border-slate-200">
      ...
      <div className="p-6 space-y-4">
        <div className="flex flex-wrap gap-2">
          ...
          <button onClick={invite} disabled={saving} className="px-3 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm inline-flex items-center gap-1.5 disabled:opacity-50">
            ...
            {collaborators.map((c) => (
              <div key={c.id} className="flex items-center gap-3 p-3 rounded-xl border border-slate-200">
                <div className="w-9 h-9 bg-indigo-100 rounded-full flex items-center justify-center shrink-0">
                  <span className="text-indigo-700 text-xs font-medium">{c.name.split(' ').map((p) => p[0]).slice(0, 2).join('').toUpperCase()}</span>
                </div>
