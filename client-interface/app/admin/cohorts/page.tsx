...
const STATUS_META: Record<CohortStatus, { label: string; cls: string }> = {
  ...
  running:   { label: 'Running',   cls: 'bg-indigo-50 text-indigo-700 border-indigo-200' },
...
function CreateCohortDrawer({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  const field = 'w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500';
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-black/30" onClick={onClose}>
      <div role="dialog" aria-modal="true" className="relative w-full max-w-md h-full bg-white shadow-xl flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-6 py-4 border-t border-slate-200 flex justify-end gap-2">
          ...
          <button onClick={submit} disabled={saving} className="inline-flex items-center gap-2 px-4 py-2 text-sm bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 text-white rounded-lg">
...
function CohortCard({ cohort }: { cohort: Cohort }) {
  ...
  return (
    <Link href={`/admin/cohorts/${cohort.id}`} className="group block rounded-2xl border border-slate-200 bg-white p-5 hover:border-indigo-300 hover:shadow-sm transition-all">
